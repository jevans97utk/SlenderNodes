...
def main():
    ...
    while (start == 1) or (start == 0 and rtoken is not None):
        ...
        if record_list is not None:
            ...
            if rtoken_record is None:
                ...
                # excludes rtoken from the processing that happens to rest of results
    ...
    tracking_log.write(datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S") + ', New Records Loaded: {}, Records Updated: {}, Records archived: {}, Deleted skipped: {}, existing skipped: {}.\n'.format(created_count, updated_count, archived_count, skipped_deleted_count, skipped_exists_count))
    tracking_log.close()
    ...
    def get_records(self, parameters):
        ...
        try:
            ...
            if r.status_code == requests.codes.ok:
                ...
                # If element not found, returns none (such as if <error code="noRecordsMatch"/>)
                return record_list
            else:
        ...
        if 'status' in record.find('{http://www.openarchives.org/OAI/2.0/}header').attrib:
            if record.find('{http://www.openarchives.org/OAI/2.0/}header').attrib['status'] == 'deleted':
                ...
                if checkExistsDict['outcome'] == 'yes':
                    ...
                    # record that a deleted record in OAI-PMH resultset was skipped over because not already in GMN
        ...
        # Then check if this identifier already exists in GMN.
            ...
            try:
                identifier = record.find('{http://www.openarchives.org/OAI/2.0/}header'). \
                  find('{http://www.openarchives.org/OAI/2.0/}identifier').text.replace(
                  'oai:pangaea.de:', '')
                record_date = record.find('{http://www.openarchives.org/OAI/2.0/}header').find('{http://www.openarchives.org/OAI/2.0/}datestamp').text
                ...
                scimeta = ET.tostring(record.find(metadata_id).find(SCIMETA_ELEMENT))
                checkExistsDict = client_mgr.check_if_identifier_exists(identifier)
            ...
            # an update. So skip it and move on.
                ...
                # identifier exists but there are no updates to apply because record date is the same
                pass

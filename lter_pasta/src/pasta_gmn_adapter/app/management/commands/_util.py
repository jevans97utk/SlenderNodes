...
def log_setup(debug_bool, show_process_and_thread=True):
  ...
  format_elememt_list = ['%(asctime)s %(levelname)-8s %(name)s']
  if show_process_and_thread:
    format_elememt_list.append('%(process)x/%(thread)x')
  format_elememt_list.append('%(module)s.%(funcName)s():%(lineno)d')
  format_elememt_list.append('%(message)s',)
  ...
  logging.config.dictConfig({
    ...
    'formatters': {
      'verbose': {
        'format': ' '.join(format_elememt_list),
        'datefmt': '%Y-%m-%d %H:%M:%S'
      },
    },
    'handlers': {

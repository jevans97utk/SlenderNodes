...
# App.
#import types.generated.xml_access
...
class EMLAccess(object):
  ...
  def deserialize(self, eml_access_xml):
    try:
      return pasta_gmn_adapter.api_types.generated.eml_access.CreateFromDocument(eml_access_xml)
  ...
  def _raise_if_access_rules_not_supported_by_dataone(self):
    ...
    if len(self._eml_access.deny) > 0:
      raise EMLAccessException('Access rules contain one or more deny rules, which are unsupported by DataONE')
  ...
  def get_as_dataone_rules(self):
    ...
    for a in self._eml_access.allow:
      ...
      for e in a.permission:
        ...
        permission = dataoneTypes.Permission(permission_str)

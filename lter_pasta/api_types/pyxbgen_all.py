...
def main():
  ...
  parser.add_option('-s', '--schemas', dest='schemas_path', action='store', type='string', default='./schemas')
  parser.add_option('-t', '--bindings', dest='bindings_path', action='store', type='string', default='./generated')
  parser.add_option('-p', '--process', dest='process_schemas', action='store', type='string', default='pasta_gmn_adapter_types.xsd;eml-access.xsd')

  (opts, args) = parser.parse_args()

  if not os.path.exists(opts.bindings_path):
    print 'The destination folder for the bindings does not exist.'
    print 'This script should be run from ./pasta_gmn_adapter/types'
    exit()

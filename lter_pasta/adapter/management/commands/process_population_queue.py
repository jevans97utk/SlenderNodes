...
class Command(NoArgsCommand):
  ...
  def handle_noargs(self, **options):
    ...
    logging.info('Running management command: ' 'process_population_queue')
  ...
  def abort_if_other_instance_is_running(self):
    single_path = os.path.join(
      tempfile.gettempdir(),
      os.path.basename(os.path.splitext(__file__)[0] + '.single')
    )
...
class PopulationQueueProcessor(object):
  ...
  def process_population_queue(self):
    ...
    package = {
     'package_scope': 'knb-lter-nin',
     'package_identifier': 18,
     'package_revision': 2,
    }
    ...
    for package in population_queue:
      try:
        ...
        data_package_manager_client.DataPackageManagerException,
        pasta_gmn_adapter.api_types.eml_access.EMLAccessException
      ) as e:
        self.logger.error('Failed: {0}'.format(str(e)))
        if e.status == 401:
          self._insert_package_processing_status(
            package, 'private', e.status,
            u'msg({0}) body({1})'.format(e.msg, e.body)
          )
        else:
          self._insert_package_processing_status(
        ...
        self._insert_package_processing_status(
          package, 'error', e.errorCode, u'description({0}) body({1})'.format(
  ...
  def _process_package(self, package):
    ...
    if previous_revision is None:
      ...
      previous_package_id = PackageID(
        package['package_scope'], package['package_identifier'], previous_revision
      )
  ...
  def _insert_package_processing_status(
    ...
    pasta_gmn_adapter.adapter.sql.insert_process_status(
      package['id'], status, return_code,
      return_body[:pasta_gmn_adapter.adapter.sql.VCHAR_LENGTH]
    )
...
class DataPackageInfoCollector(object):
  ...
  def collect_package_info(self, package_id):
    ...
    d1_replication_policy = self._pasta_client.read_metadata_replication_policy(package_id)
...
class GMNPackageCreator(object):
  ...
  def _generate_resource_map(self, package_pid, metadata_pid, entity_pids):
    ...
    return resource_map_generator.simple_generate_resource_map(
      package_pid, metadata_pid, entity_pids
    )
    ...
    if verify_checksum and not d1_common.checksum.checksums_are_equal(
      ...
      raise PopulateError(
        ...
        'pid={}, existing={}/{}, new={}/{}'.format(
          pid, sys_meta_existing.checksum.algorithm, sys_meta_existing.checksum.
          value(), sys_meta.checksum.algorithm, sys_meta.checksum.value(),
        )
      )
    return True
...
class SysmetaCreator(object):
  ...
  def create_sys_meta_for_resource(
    ...
    return self._generate_sys_meta(
      pid, size, format_id, sha1_checksum, d1_access_rules, d1_replication_policy
    )
  ...
  def _generate_replication_policy(self, d1_replication_policy):
    if d1_replication_policy is None:
      return None
    print '4'*100
    print d1_replication_policy
    return dataoneTypes.CreateFromDocument(d1_replication_policy)
...
class MediaTypeToFormatIDMapper():
  ...
  def _get_format_id_cache_file_path(self):
    return os.path.join(
      tempfile.gettempdir(), settings.FORMAT_ID_CACHE_FILENAME
    )

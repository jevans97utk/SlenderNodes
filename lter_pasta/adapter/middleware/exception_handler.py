...
class exception_handler():
  def process_exception(self, request, exception):
    ...
    if isinstance(
      exception, pasta_gmn_adapter.api_types.adapter_error.AdapterError
    ):
  ...
  def serialize_adapter_exception(self):
    ...
    return HttpResponse(
      exception_serialized, status=self.exception.http_status_code,
  ...
  def traceback_to_text(self):
    ...
    while exception_traceback:
      ...
      tb.append(
        '{0}({1})'.format(
          str(os.path.basename(co.co_filename)),
          str(traceback.tb_lineno(exception_traceback))
        )
      )
      exception_traceback = exception_traceback.tb_next
    if not isinstance(
      exception_value, d1_common.types.exceptions.DataONEException
    ):
